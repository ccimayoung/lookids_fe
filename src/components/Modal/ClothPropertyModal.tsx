...
export const ClothPropertyModal = ({
  ...
  const [wantPropertyClothId, setWantPropertyClothId] = useRecoilState(
    wantPropertyClothIdAtom,
  );
  ...
  const makeClothFunc = () => {
    if (clothInfo) {
      ...
      const colorIndex = clothInfo.colorList?.findIndex(
        (val: any) =>
          val.color === contentRef.current.children[0].children[1].innerText,
      );
      const scaleIndex = clothInfo.scaleList?.findIndex(
        (val: any) =>
          val.size === contentRef.current.children[1].children[1].innerText,
      );
  ...
  useEffect(() => {
    if (modalGather.clothProperty && wantPropertyClothId) {
      const clothIndex = clothList.findIndex(
        (val: any) => val.clothId === wantPropertyClothId,
      );
      setClothInfo(clothList[clothIndex]);
      const sizeArray: string[] = [];
      const colorArray: string[] = [];
      clothList[clothIndex]?.scaleList?.forEach(
        (val: { size: string; scale: [number, number] }) => {
          sizeArray.push(val.size);
        },
      );
      setSizeList(sizeArray);
      clothList[clothIndex]?.colorList?.forEach(
        (val: { color: string; img: string }) => {
          ...
          <ModalBox>
            ...
            <ContentWrap ref={contentRef}>
              <TitleAndSelectBox
                $title="색상"
                $titleWidth="64px"
                $content={clothInfo.colorList[0].color}
                $list={colorList}
                $contentSize="l"
              />
              <TitleAndSelectBox
                $title="사이즈"
                $titleWidth="64px"
                $content={clothInfo.scaleList[0].size}
                $list={sizeList}
                $contentSize="l"
              />
            </ContentWrap>
